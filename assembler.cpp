...
void b(vector<string>&v,vector<string>&current,map<string, int> &labelMap,int i){
    ...
    if (labelMap.find(operand) != labelMap.end()) {
        ...
        binaryValue = bitset<32>((labelMap[operand]-i*4-4)).to_string();
        current.push_back(binaryValue.substr(5));return ;
    } 
    else {
...
// cout<<"the end\n"<<endl;
    ...
    for(int i=0;i<v.size();i++){
        ...
        if(v[i][0]=="BGT"||v[i][0]=="B"||v[i][0]=="CALL"||v[i][0]=="BEQ") {ans.push_back(current);continue;}
    ...
    for(int i=0;i<v.size();i++){
        ...
        else if(v[i][0]=="CALL"||v[i][0]=="B"||v[i][0]=="BGT"||v[i][0]=="BEQ") b(v[i],ans[i],labelMap,i);
    }
...
outFile.close ();
cout << "Output successfully written to output.bin!" << endl;
    return 0;
}
